...
from PySide6.QtWidgets import (
    ...
    QAbstractItemView, QStyledItemDelegate, QMessageBox, QFrame, QFileDialog,
)
...
class EstoquePage(QWidget):
    ...
    def _exportar_excel(self):
        caminho, _ = QFileDialog.getSaveFileName(self, "Exportar Excel", "Estoque.xlsx", "Excel (*.xlsx)")
        if not caminho:
            return
        wb = Workbook()
        ws = wb.active
        ...
        for item in self.dados:
            ws.append([item.get(c, "") for c in self.CHAVES])
        wb.save(caminho)

...
class Baixa37Page(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.grafico_titulo = QLabel("Progresso da Baixa")
    ...
    def _atualizar_grafico(self):
        ...
        self.grafico_subtitulo.setText(
            f"{self._itens_concluidos} de {self._total_itens} itens" if self._total_itens else "Nenhum item para processar"
        )
        ...
        if self._total_itens == 0:
            ax.text(0.5, 0.5, "Sem itens", ha="center", va="center",
    ...
    def _colar(self):
        ...
        for linha in linhas:
            ...
            kardex    = partes[0].upper() if len(partes) > 0 else ""
            descricao = partes[1].upper() if len(partes) > 1 else ""
            qtde      = partes[2] if len(partes) > 2 else ""
            conta     = partes[3].upper() if len(partes) > 3 else ""
            subconta  = partes[4].upper() if len(partes) > 4 else ""
            secao     = partes[5].upper() if len(partes) > 5 else ""
            projeto   = partes[6].upper() if len(partes) > 6 else ""
